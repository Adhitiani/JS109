...
- for each of word:
  - check if the word is equal to the search word
  - if it's equal:
    - push the highlited word --> **HIGHLIGHTEDWORD**
  - else push the current word
...
function searchWord(searchString, text) {
  ...
  for (let i = 0; i < wordArr.length; i++) {
    if(wordArr[i].toLowerCase() === searchString) {
     result.push(`**${wordArr[i].toUpperCase()}**`)
    } else {
