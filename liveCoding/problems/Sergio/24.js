...
output: an array
the sum of the numbers that are the same and consecutives

rules/question:
- return the sum of the numbers that are the same and consecutive
- the input is never an empty array
...
- init 'sum' to 0
- iterate over the input array
  - increment sum to the curr element
  - if the current element is not the same as the next element
    - push the sum to the 'result'
    - sum = 0
- end iteration
- return 'result'
*/
...
/*
- init 'result' to [arr[0]]
- iterate over the arr
  - start the iteration at index 1
  - init 'currNum' = arr[i]
  - init 'prevNum' = arr[i - 1]
  - if 'currNum' === 'prevNum'
    - add 'currNum' to the last element of the 'result'
  - else
    - push 'currNum' to 'result'  
- end iteration
- return result
*/
...
function sumConsecutives(arr) {
  ...
  for (let i = 1; i < arr.length; i++) {
   ...
   if (currArr === prevArr) {
     result[result.length - 1] += currArr;
   } else {
     result.push(currArr);
   }
  }
