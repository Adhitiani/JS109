...
ouput: a number (the sum of its digit)
...
Alghoritm:
1. convert the number to the string and to an array. 
2. iterate over the array and for each of number convert it to a number add it with the next number

Copy Code */

function sum(num) {
  let numbers = String(num).split('');
  return numbers.reduce((sum, currNum) => sum + Number(currNum), 0);
};
