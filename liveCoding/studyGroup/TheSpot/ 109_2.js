...
function revRot(num, chunkLength) { 
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    if (cubedDigitsIsEven(chunk)) {
      // console.log(`Cubed digits were even`);
      chunks[i] = chunk.split('').reverse().join('');
    } else {
      // console.log(`Cubed digits were odd`);
      chunks[i] = rotateToLeft(chunk);
    } 
  }
...
function cubedDigitsIsEven(chunk) {
  const digits = [...chunk].map(char => Number(char));
  let sum = 0;
  
  for (let i = 0; i < digits.length; i++) {
    sum += digits[i] ** 3;
    // we needed to cube each digit (digit^3) and add it to sum
    // There are two ways to perform power operations in JavaScript: 
    //     - Math.pow(digits[i], 3);
    //     - <base> ** <power> 
  }
...
function rotateToLeft(chunk) {
  const chars = chunk.split('');
  const front = chars.shift();
  chars.push(front);
  return chars.join('');
}
...
console.log(revRot(123456779, 8) === 23456771); // --> 23456771
console.log(revRot(0, 8) === -1); // --> -1
console.log(revRot(123456779, 0) === -1); // --> -1 
console.log(revRot(123, 4) === -1) // --> -1
console.log(revRot(5630006557344, 2) === 650300567543); // --> 65 03 00 56 75 43
